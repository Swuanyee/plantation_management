# File: plantation_management/plantation_management/report/plantation_summary/plantation_summary.py
...
def get_data(filters):
    ...
    for r in rows:
        ...
        # First row: initialize grouping
        if current_division is None:
            current_division = div
            current_district = dist
            current_township = town
            current_parent = None  # will be set below

        # Handle group changes (outermost -> inner)
    ...
    # Flush remaining groups at the end
    ...
    # Grand total row (all Plantation Records)
    if data:
        data.append({
            "reporting_division": "",
            "reporting_district": "",
