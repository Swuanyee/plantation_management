...
fields = {
    ...
    "area_of_destruction": "ပျက်စီးမှု ဧရိယာ (acre in Burmese)",
...
def get_columns():
    return [
        ...
        },
        {
            ...
            "fieldtype": "Data",  # already stored as Burmese digits
            "width": 140,
        },
        {
            # Link to Annual Plantation Update; blank for rows that come only from Disease Observed Location
...
def get_data(filters):
    ...
    disease_filter = normalize_list_filter(filters.get("disease"))
    ...
    # Disease filter (applies to both)
    if disease_filter:
        obs_conds.append("d.name IN %(disease)s")
        ann_conds.append("d.name IN %(disease)s")
        params["disease"] = tuple(disease_filter)
    ...
    # -----------------------------------
    # 1) From Disease → Child Observed Location (no date, no APU link, no area)
    # -----------------------------------
    observed_query = f"""
        SELECT
            d.name AS disease,
        ...
        JOIN `tabChild Observed Location` col
            ON col.parent = d.name
           AND col.parenttype = 'Disease'
        WHERE 1 = 1
        {obs_where}
    """
    ...
    # -----------------------------------
    # 2) From Annual Plantation Update → Child Disease Destruction Table → Plantation Record
    # -----------------------------------
    annual_query = f"""
        SELECT
            d.name AS disease,
    ...
    for r in annual_rows:
        ...
        area_burmese = r.get("area_of_destruction") or ""
        data.append({
            "disease": r.get("disease"),
            "reporting_division": r.get("reporting_division"),
            "reporting_district": r.get("reporting_district"),
